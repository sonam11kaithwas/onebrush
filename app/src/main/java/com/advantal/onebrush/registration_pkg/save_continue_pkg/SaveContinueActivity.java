...
import com.advantal.onebrush.databinding.ActivitySaveContinueBinding;
...
import com.advantal.onebrush.registration_pkg.save_continue_pkg.save_continue_viewmodel_pkg.Save_Continue_Factory;
import com.advantal.onebrush.registration_pkg.save_continue_pkg.save_continue_viewmodel_pkg.Save_Continue_View_Model;
...
public class SaveContinueActivity extends AppCompatActivity {
    private final boolean CHECKSTATE = false;
    List<Country_Model> list;
    private Save_Continue_View_Model saveContinueViewModel;
    private ActivitySaveContinueBinding binding;
...
//
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_save_continue);

        list = new ArrayList<>();

        saveContinueViewModel = ViewModelProviders.of(this, new Save_Continue_Factory(this,
                new Save_Continue_Model())).get(Save_Continue_View_Model.class);
        binding.setContinueModel(saveContinueViewModel);
        saveContinueViewModel.getCountryCode();
        ...
        AppUtility.updateScreeState("SaveContinueActivity", userAccountDetailsModel);

        setALreadyFillValue();

        saveContinueViewModel.getUserRegisteredSuccessFully().observeForever(new Observer<String>() {
            @Override
            public void onChanged(String s) {
                AppUtility.hideProgressBar(SaveContinueActivity.this);
        ...
        saveContinueViewModel.getErrorMsgBar().observeForever(new Observer<ErrorModel>() {
            @Override
        ...
        saveContinueViewModel.getUserCountryList().observeForever(new Observer<List<Country_Model>>() {
            @Override
    ...
    private void setALreadyFillValue() {
        if (userAccountDetailsModel != null) {
            if (userAccountDetailsModel.getStreetAndHousenumber() != null)
                binding.etStreet.setText(userAccountDetailsModel.getStreetAndHousenumber());
            if (userAccountDetailsModel.getZipcode() != null)
                binding.etZipcode.setText(userAccountDetailsModel.getZipcode());
            if (userAccountDetailsModel.getCity() != null)
                binding.etCity.setText(userAccountDetailsModel.getCity());
            if (userAccountDetailsModel.getCountry() != null)
                binding.etCountry.setText(userAccountDetailsModel.getCountry());
            if (userAccountDetailsModel.getTelefon() != null)
                binding.etPhone.setText(userAccountDetailsModel.getTelefon());
    ...
    private void startNewActionForUser() {
        ...
        Intent intent = new Intent(this, ContinueWithPaymentActivity.class);
    ...
    public void onClickButton(View view) {
        switch (view.getId()) {
            ...
            case R.id.back_button:
                Intent intent = new Intent(this, Registr_NextActivity.class);
                intent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
                startActivity(intent);
                finish();
                break;
    ...
    private void opennewCountryDialog() {
        countryDialog = new Dialog(SaveContinueActivity.this);
        ...
        if (!binding.etStreet.getText().toString().trim().isEmpty()) {
            if (!binding.etZipcode.getText().toString().trim().isEmpty()) {
                if (!binding.etCity.getText().toString().trim().isEmpty() &&
                        Pattern.matches("[a-zA-Z]+", binding.etCity.getText().toString().trim())
                ) {
                    if (!binding.etCountry.getText().toString().trim().isEmpty() &&
                            Pattern.matches("[a-zA-Z]+", binding.etCountry.getText().toString().trim())) {
                        if (msg.isEmpty()) {
                            userAccountDetailsModel.setZipcode(binding.etZipcode.getText().toString().trim());
                            userAccountDetailsModel.setStreetAndHousenumber(binding.etStreet.getText().toString().trim());
                            userAccountDetailsModel.setCity(binding.etCity.getText().toString().trim());
                            userAccountDetailsModel.setCountry(binding.etCountry.getText().toString().trim());
                            userAccountDetailsModel.setTelefon(binding.etPhone.getText().toString().trim());
                            userAccountDetailsModel.setTelefonIsoCode(binding.tvTextPhone.getText().toString().trim());
                            ...
                            if (AppUtility.isNetworkConnected()) {
                                AppUtility.showProgressBaForRelLay(binding.rlayoutParant, this);
                                saveContinueViewModel.userRegistred(userAccountDetailsModel);
                            } else {
